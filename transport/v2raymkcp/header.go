...
func newPacketHeader(name string) packetHeader {
	...
	case "", "none", "noop":
		return noopHeader{}
	case "srtp":
		return &srtpHeader{header: 0xb5e8, number: uint16(rand.Uint64() >> 47)}
	case "utp":
		return &utpHeader{header: 1, connectionID: uint16(rand.Uint64() >> 47)}
	case "wechat-video", "wechat":
		return &wechatVideoHeader{sn: uint32(uint16(rand.Uint64() >> 47))}
	case "dtls":
		return &dtlsHeader{epoch: uint16(rand.Uint64() >> 47), length: 17}
	case "wireguard":
