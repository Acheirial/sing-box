...
func newConfig(options option.V2RayXHTTPOptions) (*Config, error) {
	...
	if options.ReuseSettings != nil {
		cfg.ReuseConfig = &ReuseConfig{
			MaxConcurrency:   options.ReuseSettings.MaxConcurrency,
			MaxConnections:   options.ReuseSettings.MaxConnections,
			CMaxReuseTimes:   options.ReuseSettings.CMaxReuseTimes,
			HMaxRequestTimes: options.ReuseSettings.HMaxRequestTimes,
			HMaxReusableSecs: options.ReuseSettings.HMaxReusableSecs,
		}
	}
	return cfg, nil
}
...
func (c *Config) GetRequestHeaderWithPayload(payload []byte, uplinkChunkSize Range) http.Header {
	...
	for i := 0; len(encodedData) > 0; i++ {
		chunkSize := min(uplinkChunkSize.Rand(), len(encodedData))
...
func (c *Config) GetGenerateSessionID() (func() string, error) {
	...
	default: // https://github.com/XTLS/Xray-core/pull/6258
		...
		if room.Cmp(big.NewInt(2<<30)) < 0 {
			return nil, fmt.Errorf("session-table or session-length is too small")
		}
		...
		return func() string {
			...
			id := make([]byte, length)
			for i := range id {
				id[i] = sessionTable[mrand.IntN(len(sessionTable))]
			}
			return string(id)
		}, nil
	}
