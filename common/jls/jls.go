...
func NewClient(ctx context.Context, conn net.Conn, config *ClientConfig) (net.Conn, error) {
	...
	tlsConn := jls.Client(conn, &jls.Config{
		ServerName: config.ServerName,
		NextProtos: append([]string(nil), alpn...),
		RootCAs:    adapter.RootPoolFromContext(ctx),
		Time:       time.Now,
		JLSConfig: &jls.JLSConfig{
			Enable: true,
			User:   config.User,
		},
	})
	...
	if tlsConn.ConnectionState().JLS.Status != jls.JLSAuthenticated {
...
func NewServerConfig(sni, dest string, users []User, alpn []string, rateLimit uint64, dialContext func(context.Context, string, string) (net.Conn, error)) (*ServerConfig, error) {
	...
	certificate, err := newRandomCertificate()
	if err != nil {
		return nil, fmt.Errorf("jls: generate TLS certificate: %w", err)
	}
	return &ServerConfig{
		TLSConfig: &jls.Config{
			Certificates: []jls.Certificate{certificate},
			NextProtos:   append([]string(nil), alpn...),
...
func newRandomCertificate() (jls.Certificate, error) {
	key, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	if err != nil {
		return jls.Certificate{}, err
	}
	template := x509.Certificate{
		SerialNumber: big.NewInt(1),
		Subject:      pkix.Name{CommonName: "jls"},
	...
	return jls.Certificate{Certificate: [][]byte{certDER}, PrivateKey: key, Leaf: leaf}, nil
}
...
func Server(ctx context.Context, conn net.Conn, config *ServerConfig) (net.Conn, error) {
	...
	tlsConn := jls.Server(recorder, config.TLSConfig.Clone())
	if err := tlsConn.HandshakeContext(ctx); err != nil {
...
func relayFallback(ctx context.Context, inbound net.Conn, prefix []byte, config *ServerConfig) error {
	...
	if len(prefix) > 0 {
		buffer := buf.NewSize(len(prefix))
		buffer.Write(prefix)
		inbound = bufio.NewCachedConn(inbound, buffer)
	}
	upstream = newRateLimitedConn(upstream, config.RateLimit)
	if err = bufio.CopyConn(ctx, inbound, upstream); err != nil {
		return err
	}
