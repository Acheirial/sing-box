...
var jlsClientFingerprints = map[string]utls.ClientHelloID{
	...
	"random":  utls.HelloChrome_Auto,
}

func jlsClientHelloID(name string) (utls.ClientHelloID, bool) {
	if name == "" || name == "none" {
		return utls.ClientHelloID{}, false
	}
	if fingerprint, ok := jlsClientFingerprints[name]; ok {
...
func newUTLSClient(ctx context.Context, conn net.Conn, config *ClientConfig) (net.Conn, bool, error) {
	fingerprint, ok := jlsClientHelloID(config.ClientFingerprint)
	if !ok {
	...
	fakeRandom, err := jlsBuildFakeRandom(config.User, hello.Random[:jlsRandomSeedLen], authData, cryptorand.Reader)
	if err != nil {
		return nil, true, err
	}
	...
	if !verifier.authenticated {
		...
		// synchronous because the caller closes conn on return.
		jlsClientHTTPFallback(ctx, uConn, config.ServerName, fingerprint.Client)
...
func jlsClientHTTPFallback(ctx context.Context, uConn *utls.UConn, serverName string, userAgent string) {
...
func (v *utlsJLSVerifier) VerifyConnection(state utls.ConnectionState) error {
	...
	if !bytes.Equal(v.clientHello, v.HandshakeState.Hello.Raw) {
		return v.verifyUTLSCertificate(state)
	}
...
func (v *utlsJLSVerifier) verifyUTLSCertificate(state utls.ConnectionState) error {
