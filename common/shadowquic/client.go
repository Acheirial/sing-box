...
func (c *Client) getConn(ctx context.Context) (*connState, error) {
	...
	quiccongestion.SetCongestionController(quicConn, c.option.CongestionController, c.option.CWND, c.option.BBRProfile)
...
func (c *Client) negotiateBrutal(state *connState) {
	...
	if !rxAuto && actualTx > 0 {
		quiccongestion.SetBrutalCongestionController(state.quicConn, actualTx)
	} else {
		quiccongestion.SetCongestionController(state.quicConn, "bbr", c.option.CWND, c.option.BBRProfile)
	}
}
...
func (c *Client) DialContext(ctx context.Context, destination M.Socksaddr) (net.Conn, error) {
	...
	target, err := MetadataAddr(destination)
	if err != nil {
		return nil, err
	}
	stream, err := state.quicConn.OpenStreamSync(ctx)
