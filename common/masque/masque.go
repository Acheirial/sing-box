// Package masque is a port of mihomo's MASQUE (Connect-IP) client transport,
// based on the usque client implementation.
package masque
...
// IpConn is a bidirectional IP packet connection over a MASQUE tunnel.
...
func dialEx(ctx context.Context, conn *http3.ClientConn, template *uritemplate.Template, requestProtocol string, additionalHeaders http.Header, ignoreExtendedConnect bool) (*connectip.Conn, *http.Response, error) {
	...
	return connectip.NewProxiedConn(&proxiedStream{RequestStream: rstr}), rsp, nil
}
