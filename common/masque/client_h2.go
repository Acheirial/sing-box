...
// ConnectTunnelH2 establishes a CONNECT-IP tunnel over HTTP/2. The connection
// factory must return a TLS connection negotiated to the h2 ALPN.
func ConnectTunnelH2(ctx context.Context, h2Transport *http2.Transport, dialConn func(ctx context.Context) (net.Conn, error), connectUri string) (io.Closer, IpConn, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("connect-ip: failed to dial h2 connection: %w", err)
	}
...
start:
	...
	if err := c.handleIncomingProxiedPacket(data); err != nil {
		log.Printf("connect-ip: dropping proxied packet: %s", err)
		goto start
	}
	return data, nil
}
...
func (c *h2IpConn) WritePacket(b []byte) (icmp []byte, err error) {
	...
	if err != nil {
		log.Printf("connect-ip: dropping proxied packet (%d bytes) that can't be proxied: %s", len(b), err)
		return nil, nil
	}
