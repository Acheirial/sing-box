...
func (a *authChainA) initUserData() {
	...
	if len(params) > 1 {
		if userID, err := strconv.ParseUint(params[0], 10, 32); err == nil {
			...
			log.Warn("Wrong protocol-param for ", a.salt, ", only digits are expected before ':'")
		}
	}
	if len(a.userKey) == 0 {
		a.userKey = a.Key
...
func (a *authChainA) PacketConn(c N.NetPacketConn) N.NetPacketConn {
	p := &authChainA{
		Base:     a.Base,
		salt:     a.salt,
		userData: a.userData,
	}
	return &PacketConn{NetPacketConn: c, Protocol: p}
}
...
func (a *authChainA) Decode(dst, src *bytes.Buffer) error {
	...
	for src.Len() > 4 {
		macKey := make([]byte, 0, len(a.userKey)+4)
		macKey = append(macKey, a.userKey...)
		macKey = binary.LittleEndian.AppendUint32(macKey, a.recvID)
...
func (a *authChainA) DecodePacket(b []byte) ([]byte, error) {
	...
	key := kdfBase64(a.userKey, md5Data)
...
func (a *authChainA) packAuthData(poolBuf *bytes.Buffer, data []byte) {
	...
	*/
	...
	macKey := make([]byte, 0, len(a.iv)+len(a.Key))
	macKey = append(macKey, a.iv...)
	macKey = append(macKey, a.Key...)
...
func (a *authChainA) packData(poolBuf *bytes.Buffer, data []byte) {
	...
	macKey := make([]byte, 0, len(a.userKey)+4)
	macKey = append(macKey, a.userKey...)
	macKey = binary.LittleEndian.AppendUint32(macKey, a.packID)
	a.packID++
...
func (a *authChainA) initRC4Cipher() {
	key := kdfBase64(a.userKey, a.lastClientHash)
...
// kdfBase64 derives a 16-byte RC4 key from two base64-encoded key parts.
func kdfBase64(part1, part2 []byte) []byte {
	return kdf(base64.StdEncoding.EncodeToString(part1)+base64.StdEncoding.EncodeToString(part2), 16)
}
