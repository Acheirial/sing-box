...
func TestMLDSA65Verifier(t *testing.T) {
	...
	*(*([]*x509.Certificate))(unsafe.Add(unsafe.Pointer(verifier.Conn), p.Offset)) = []*x509.Certificate{cert}
...
func TestMLDSA65Verifier_X25519MLKEM768(t *testing.T) {
	pub, priv, err := mldsa65.GenerateKey(nil)
	require.NoError(t, err)
	pubBinary, _ := pub.MarshalBinary()
	...
	*(*([]*x509.Certificate))(unsafe.Add(unsafe.Pointer(verifier.Conn), p.Offset)) = []*x509.Certificate{cert}
	...
	verifier.HandshakeState.ServerHello.ServerShare.Group = utls.X25519MLKEM768
